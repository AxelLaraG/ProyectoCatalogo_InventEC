...
class CatalogoGUI:
    ...
    def crear_widgets(self):
        # Título
        titulo = tk.Label(self.master, text="Catálogo de Equipamiento Audiovisual", font=("Segoe UI", 18, "bold"), bg="#F5F7FA", fg="#2C3E50")
        titulo.pack(pady=10)
        ...
        self.campo_busqueda = ttk.Combobox(marco_busqueda, values=["nombre", "modelo", "numero_serie", "estado", "ubicacion", "todos"], state="readonly")
        self.campo_busqueda.set("todos")
        self.campo_busqueda.grid(row=0, column=3, padx=5)

        # Botones
        btn_buscar = tk.Button(marco_busqueda, text="Buscar", command=self.buscar_equipo, bg="#3498DB", fg="white", font=("Segoe UI", 10), relief="flat")
        btn_buscar.grid(row=0, column=4, padx=5)
        btn_todos = tk.Button(marco_busqueda, text="Mostrar Todos", command=self.cargar_equipos, bg="#1ABC9C", fg="white", font=("Segoe UI", 10), relief="flat")
        btn_todos.grid(row=0, column=5, padx=5)

        # Filtro por estado separado (opcional)
        ...
        # Tabla
        columnas = ("_id", "nombre", "modelo", "numero_serie", "estado", "ubicacion")
        self.tree = ttk.Treeview(self.master, columns=columnas, show="headings", height=15)
    ...
    def buscar_equipo(self):
        ...
        campo = self.campo_busqueda.get().strip().lower()
        self.limpiar_tabla()
